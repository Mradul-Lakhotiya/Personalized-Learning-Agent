...
export function NodeDetailPanel({ node, onClose, onMarkComplete, sessionToken, userId, threadId }) {
  ...
  const handleGenerateResources = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/users/${userId}/conversations/${threadId}/nodes/${node.id}/generate`, {
        headers: {
      ...
      if (!response.ok) {
        console.error('Generation request failed with status:', response.status);
        ...
        for (const msg of messages) {
          ...
          try {
            ...
            if (eventData.type === 'status') {
              ...
              // Fetch the newly generated data from Node API
              try {
                const res = await fetch(`${API_BASE_URL}/api/v1/users/${userId}/conversations/${threadId}/nodes/${node.id}`);
                if (res.ok) {
                  const newData = await res.json();
                  setResources(newData.resources_cached || []);
                  setQuestions(newData.questions_cached || []);
                }
              } catch (e) {
  ...
  useEffect(() => {
    ...
    const fetchOnly = async () => {
      ...
      // If we got here, we didn't find cached data.
      setLoadingResources(false);
      if (node.is_major) {
        setNeedsGeneration(true);
      }
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try {
      // Call the Node.js backend to mark as complete
      const res = await fetch(`${API_BASE_URL}/api/v1/users/${userId}/conversations/${threadId}/nodes/${node.id}/complete`, {
          method: 'POST',
          headers: {
      ...
      if (res.ok) {
          ...
          console.error('Failed to complete node via Node API');
      }
    } catch (e) {
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 bg-surface-container-lowest space-y-6">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-secondary-container text-on-secondary-container font-label-sm text-label-sm mb-3">
              <span className="material-symbols-outlined text-[14px]">
                {isCompleted ? 'check_circle' : (node.status === 'in_progress' ? 'play_circle' : 'lock')}
              </span>
              {isCompleted ? 'Completed' : (node.status === 'locked' ? 'Locked' : 'Available')}
            </div>
          ...
          {resources && resources.length > 0 ? (
                  ...
                  <li key={i}>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <span className="block font-label-sm text-label-sm text-on-surface-variant truncate w-[280px]">
                          {r.url}
                        </span>
                      </div>
                    </a>
            ...
            <div className="p-5 bg-surface-container border border-outline-variant rounded-xl text-center shadow-sm">
               ...
               <p className="font-label-sm text-label-sm text-on-surface-variant mb-5">Click below to trigger our AI Swarm. It will scour the internet to curate resources and synthesize custom quiz questions specifically for {node.title}.</p>
               <button 
